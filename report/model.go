...
type DonateItem struct {
	...
	DonateID    uuuid.UUID        `bson:"donateID,omitempty" json:"donateID,omitempty"`
...
type DonateItemParams struct {
	Timestamp *Comparator `json:"timestamp,omitempty"`
}

func (s DonateItem) MarshalBSON() ([]byte, error) {
	si := map[string]interface{}{
		"itemID":      s.ItemID.String(),
		"donateID":    s.DonateID.String(),
		"lot":         s.Lot,
		"name":        s.Name,
		"sku":         s.SKU,
		"weight":      s.Weight,
...
func (s DonateItem) UnmarshalBSON(in []byte) error {
...
func (s DonateItem) unmarshalFromMap(m map[string]interface{}) error {
	var err error
	var assertOK bool
	...
	if m["donateID"] != nil {
		s.DonateID, err = uuuid.FromString(m["donateID"].(string))
		if err != nil {
